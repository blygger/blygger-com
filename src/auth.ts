// Owner-only cookie auth. Lifted verbatim from blygger-spec/worker/src/auth.ts
// (same owner model, same threat model, no reason to write a second one) with
// only the cookie name changed so a session on one does not read as a session
// on the other.
...
export const COOKIE_NAME = "blygdir_session";
